...
class UpdateManager:
	...
	def log_update(self, entity_type: str, entity_id: str, change_type: str, changes: Dict[str, Any]) -> UpdateHistory:
	...
	def update_album(self, album_id: str, updates: Dict[str, Any]) -> Optional[Album]:
		...
		if changes:
			self.log_update('album', album_id, 'update', changes)
			self.db.commit()
	...
	def update_tag(self, tag_id: str, updates: Dict[str, Any]) -> Optional[Tag]:
		...
		changes = self.track_changes(old_data, updates)
		if changes:
			self.log_update('tag', tag_id, 'update', changes)
			self.db.commit()
			self.db.refresh(tag)
